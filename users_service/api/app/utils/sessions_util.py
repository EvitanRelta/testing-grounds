...
def create_session(user_id, role):
    ...
    db.execute_sql_write(
        "INSERT INTO sessions (session_id, user_id, role, creation_time, expiration_time) VALUES (%s, %s, %s, %s, %s)",
...
def is_logged_in(session_id):
    return (
        db.execute_sql_read_fetchone(
            "SELECT COUNT(*) FROM sessions WHERE session_id = %s", params=(session_id,)
        )[0]
        > 0
    )
