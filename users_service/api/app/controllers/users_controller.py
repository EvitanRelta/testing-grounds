...
def create_user(user_id, username, email, password):
    if users_util.uid_exists(user_id):
        return users_util.http_exception_message(
            status_code=500, message="Internal server error (uid already exists)"
        )
...
def update_user_role(user_id, role):
    if role == "normal":
        if (
            db.execute_sql_read_fetchone("SELECT COUNT(*) FROM users WHERE role = 'maintainer'")[0]
            <= 1
        ):
