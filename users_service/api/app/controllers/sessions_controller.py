...
def user_login(username: str, password: str):
    ...
    if login_result:
        ...
        return {"session_id": session_id, "message": f"User {username} successfully logged in"}
    else:
        ...
        return sessions_util.http_exception_message(
            status_code=401, message="Account does not exist"
        )
...
def get_session(session_id):
    ...
    result = db.execute_sql_read_fetchone(
        "SELECT * FROM sessions WHERE session_id = %s", params=(session_id,)
    )
...
def user_logout(session_id):
    try:
        ...
        return sessions_util.http_exception_message(
            status_code=500, message=f"Unable to logout user: {e}"
        )
