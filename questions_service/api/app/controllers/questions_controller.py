...
def create_question(question_id, title, description, category, complexity):
    if not questions_util.is_valid_complexity(complexity):
        return questions_util.http_exception_message(
            status_code=422,
            message="Invalid value for complexity. Complexity must only be Easy, Medium, or Hard",
        )
    if questions_util.qid_exists(question_id):
        return questions_util.http_exception_message(
            status_code=500, message="Internal server error (qid already exists)"
        )
    if questions_util.title_exists(title):
        return questions_util.http_exception_message(
            status_code=409, message="Title already exists"
        )
...
def get_question(question_id):
    if not questions_util.qid_exists(question_id):
        return questions_util.http_exception_message(
            status_code=404, message="Question id does not exist"
        )
...
def update_question_info(question_id, title, description, category, complexity):
    if not questions_util.qid_exists(question_id):
        return questions_util.http_exception_message(
            status_code=404, message="Question does not exist"
        )
